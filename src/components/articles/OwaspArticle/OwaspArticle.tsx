  ...
  return (
    <div className={s.article}>
      ...
      <section className={s.section}>
        <SectionTitle>OWASP Top 10: почему это важно</SectionTitle>
        <p className={s.lead}>
      ...
      <section className={s.section}>
        <SectionTitle>A01: Broken Access Control</SectionTitle>
        <p className={s.lead}>
        ...
        <CodeHighlight lang="ts" code={`// ❌ Проверяет только "залогинен ли", но не "его ли это заказ"
...
const order = await db.query(
  ...
  [id, req.user.sub]   // если чужой — 0 строк → 404
        ...
        <CodeHighlight lang="ts" code={`// ❌ Проверяем аутентификацию, но не роль
...
app.delete('/api/users/:id', authMiddleware, async (req, res) => {
  ...
  res.status(204).send();
      ...
      <section className={s.section}>
        <SectionTitle>A02: Cryptographic Failures</SectionTitle>
        <p className={s.lead}>
        ...
        <CodeHighlight lang="ts" code={`const bcrypt = require('bcrypt');
...
if (!valid) return res.status(401).json({ error: 'Invalid credentials' });`} />
        ...
        <CodeHighlight lang="ts" code={`// ❌ Секреты в коде — попадают в git историю
...
// Если secret уже в истории — его нужно считать скомпрометированным`} />
      </section>

      {/* ── 4. A03 Injection ────────────────────────────────────────────────── */}
      <section className={s.section}>
        <SectionTitle>A03: Injection</SectionTitle>
        <p className={s.lead}>
        ...
        <CodeHighlight lang="ts" code={`const { exec } = require('child_process');
...
app.post('/convert', (req, res) => {
  ...
  execFile('convert', [filename, 'output.png'], (err, out) => {
    res.send(out);
      ...
      {/* ── 5. VulnExplorer ─────────────────────────────────────────────────── */}
      <section className={s.section}>
        <SectionTitle>Атака и защита: интерактивный разбор</SectionTitle>
        <p className={s.body}>
      ...
      {/* ── 6. XSS ──────────────────────────────────────────────────────────── */}
      <section className={s.section}>
        <SectionTitle>XSS: три вида и одна защита</SectionTitle>
        <p className={s.body}>
        ...
        <CodeHighlight lang="ts" code={`// ✓ React/Vue/Angular: { variable } — автоматическое экранирование
...
const clean = sanitizeHtml(userHtml, { allowedTags: ['b', 'i', 'a'] });  // server`} />
      </section>

      {/* ── 7. Security Misconfiguration ────────────────────────────────────── */}
      <section className={s.section}>
        <SectionTitle>A05: Security Misconfiguration</SectionTitle>
        <p className={s.body}>
      ...
      {/* ── 8. Auth Failures ────────────────────────────────────────────────── */}
      <section className={s.section}>
        <SectionTitle>A07: Authentication Failures</SectionTitle>
        <CodeHighlight lang="ts" code={`const rateLimit = require('express-rate-limit');
...
// Использовать zxcvbn для оценки силы пароля`} />
      </section>

      {/* ── 9. Security Headers ─────────────────────────────────────────────── */}
      <section className={s.section}>
        <SectionTitle>Security Headers: быстрые wins</SectionTitle>
        <p className={s.body}>
        ...
        <CodeHighlight lang="ts" code={`const helmet = require('helmet');
app.use(helmet()); // устанавливает все заголовки ниже автоматически`} />
        <div className={s.tableWrap}>
          <table className={s.table}>
            <thead>
              <tr>
      ...
      {/* ── 10. Чеклист ─────────────────────────────────────────────────────── */}
      <section className={s.section}>
        <SectionTitle>Чеклист: минимум безопасности</SectionTitle>
        <div className={s.twoCols}>
          <div className={s.colCard}>
